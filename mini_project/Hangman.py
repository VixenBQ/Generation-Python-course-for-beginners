...
def get_world():
    a = random.randrange(0, len(word_rus))
    return word_rus[a].upper()
...
def play(word):
    ...
    steps_to_win = len(word)
    print('Сыграем в виселицу!')
    while tries != 0 and steps_to_win != 0:  # Пока не закончились попытки
        print(display_hangman(tries))
        print(word_completion + ' - загаданное слово')
        letter = input('Введите букву, или целое слово: ').upper()
        while letter.isalpha() != True or len(letter) == 2:
            print('Пожалуйста, введите ОДНУ букву - цифры и символы не подходят!')
            letter = input('Введите букву, или целое слово: ').upper()
        if len(letter) > 2 and letter == word:
            print(f'Вы угадали слово! Слово было {word}')
            break
        elif len(letter) > 2 and letter != word:
            print('Неправильно! Вы не угадали, все попытки сгорели. Вы проиграли!')
            print(f'Слово было {word}')
            break
        if letter in storage:
            print('Эта буква уже была!')
            print('Использованные буквы: ', *storage)
            continue
        elif letter not in storage and letter in word:
            # ВНИМАНИЕ! Работающий цикл по добавлению нужной буквы:
            ...
            print('Использованные буквы: ', *storage)
        elif letter not in storage and letter not in word:
            tries -= 1
            storage.append(letter)
            print('Не угадали!')
            print('Использованные буквы: ', *storage)
    ...
    if steps_to_win == 0:
        print(display_hangman(tries))
        print(f'Вы выиграли! Слово было {word}')

    # Вопрос о повторной игре
    answer = input('Сыграем еще раз? Да \ нет? :')
    while answer.lower() in ('да', 'д', "y", 'yes'):
        word_rus.remove(word.lower())
        print(f'Слова которые уже были: {guessed_words}')
        play(get_world())
        answer = input('Сыграем еще раз? Да \ нет? :')
